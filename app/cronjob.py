...
def binary_split_resend(rows_quarantined):
        ...
        for x in [0, 1]:
            if x==0:
                ...
                LOG.info("Binary split in pass: "+str(x)+" from 0 to " + str(round(len(rows_quarantined)/2))+ " total array length: "+str(len(rows_quarantined)) )
            else:
                start=round(len(q_transactions)/2)
                stop=len(q_transactions)
                q_half_array = q_transactions[round(len(rows_quarantined)/2):]
                LOG.info("Binary split in pass: "+str(x)+" from "+ str(round(len(rows_quarantined)/2)) + " to " + str(len(rows_quarantined))+ " total array length: "+str(len(rows_quarantined)) )
            ...
            if tx_id:
                ...
                LOG.info("Binary split failed: on pass: "+ str(x) +" start: " + str(start) + " stop: "+ str(stop) + " length: "+ str(len(subset_of_rows) )+" of original: "+str(len(rows_quarantined)) )
                
                if len(subset_of_rows)>=2:
                    LOG.info("Binary split IF:  start " + str(start) + " stop: "+ str(stop) + " length: "+ str(len(subset_of_rows)) )
